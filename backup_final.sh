...
if [[ "$targetDirectory" == *"/"* ]]
then
origAbsPath=$targetDirectory
else
origAbsPath=$origAbsPath/$targetDirectory
end
if [[ "$destinationDirectory" == *"/"* ]]
then
destDirAbsPath=$destinationDirectory
else
destDirAbsPath=$destDirAbsPath/$destinationDirectory
end
...
declare -a ListFiles
ListFiles+=(`ls $origAbsPath/*`) 

declare -a toBackup
for file in ${ListFiles[@]}  # [TASK 9]
do
